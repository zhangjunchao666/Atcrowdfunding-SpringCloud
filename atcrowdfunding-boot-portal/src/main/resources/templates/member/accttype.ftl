...
.seltype {
 ...
 <div class="navbar-wrapper">
	...
	<button id="accttypeApply" type="button" class="btn btn-danger btn-lg btn-block" >认证申请</button>
    </div> <!-- /container -->
      <!-- /END THE FEATURETTES -->
    ...
    $("#accttypeApply").click(function(){
		//1.先选择图片（选择账户类型）
		//2.点击【认证申请】
		
		if(accttype == null){
			//①如果没有选择账户类型，给予友好提示
			layer.msg("请先选择账户类型,再开始认证申请！", {time:1000, icon:5, shift:6});
			return false ;
		}else{
			...
			$.ajax({
               type : "POST",
               url  : "${APP_PATH}/member/accttypeApply",
               data : {
                   accttype : accttype
               },
